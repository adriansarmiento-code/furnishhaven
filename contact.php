...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        // 1. Send email (recommended)
        $to = 'support@furnishhaven.com';
        $headers = "From: $email\r\n";
        ...
        // 2. Optional: Store in database
        
        try {
            $pdo = new PDO("mysql:host=$db_host;dbname=$db_name", $db_user, $db_pass);
            $stmt = $pdo->prepare("INSERT INTO messages (name, email, subject, message) VALUES (?, ?, ?, ?)");
            $stmt->execute([$name, $email, $subject, $message]);
        } catch (PDOExcepti on $e) {
            ...
            <div class="contact-form">
                ...
                <form method="POST" action="contact.php">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="subject" name="subject" class="form-control" value="<?php echo isset($subject) ? $subject : ''; ?>" required>
                    </div>
                    
                    <div class="form-group">
                        <label for="message" class="form-label">Message</label>
                        <textarea id="message" name="message" class="form-control" required><?php echo isset($message) ? $message : ''; ?></textarea>
                    </div>
